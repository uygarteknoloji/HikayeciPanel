...
procedure TCihazForm.btnKapatClick(Sender: TObject);
begin
  try
...
function TCihazForm.GenerateGuid: string;
var
  s: string;
begin
...
procedure TCihazForm.btnKaydetClick(Sender: TObject);
begin
  try
    if qCihaz.State in dsEditModes then
    begin
...
procedure TCihazForm.btnSilClick(Sender: TObject);
begin
  try
    sil.Show;
  except on E: Exception do
...
procedure TCihazForm.btnVazgecClick(Sender: TObject);
begin
  try
    if qCihaz.State in dsEditModes then
    begin
...
procedure TCihazForm.btnYeniClick(Sender: TObject);
begin
  try
...
procedure TCihazForm.dsCihazStateChange(Sender: TObject);
begin
  DugmeleriAyarla;
end;

procedure TCihazForm.silConfirm(Sender: TObject);
begin
  try
    qCihaz.Delete;
    if PCihazlarForm<>nil then
...
procedure TCihazForm.UniFrameAjaxEvent(Sender: TComponent; EventName: string;
  Params: TUniStrings);
begin
  if EventName = 'esc_pressed' then btnKapatClick(Sender);
end;

procedure TCihazForm.UniFrameCreate(Sender: TObject);
begin
...
procedure TCihazForm.DugmeleriAyarla;
begin
  try
...
procedure TCihazForm.iptalConfirm(Sender: TObject);
begin
  try
    qCihaz.Cancel;
    if PCihazlarForm<>nil then
