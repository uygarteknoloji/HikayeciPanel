...
type
  TReklamlarForm = class(TUniFrame)
    UniPanel1: TUniPanel;
    ...
    Grid: TUniDBGrid;
    dsReklamlar: TDataSource;
    sil: TUniSweetAlert;
    qReklamlar: TUniQuery;
...
uses ServerModule, MainModule, Utils, Reklam;

procedure TReklamlarForm.btnBulClick(Sender: TObject);
begin
  try
    qReklamlar.Close;
    qReklamlar.Conditions.Clear;
    pnlBul.Caption := '';
    Grid.WebOptions.PageSize := StrToInt(txtKayitSayisi.Text);
    if txtDurum.ItemIndex=0 then
    begin
      qReklamlar.Conditions.Add('DURUM', 'R_DURUM=1', True);
      pnlBul.Caption := pnlBul.Caption + ifthen(pnlBul.Caption <> '', ', ') + 'Durum : Kullanýmda';
    end
    else
    if txtDurum.ItemIndex=1 then
    begin
      qReklamlar.Conditions.Add('DURUM', 'R_DURUM=0', True);
      pnlBul.Caption := pnlBul.Caption + ifthen(pnlBul.Caption <> '', ', ') + 'Durum : Kullaným Dýþý';
    end;
    if txtDosya.Text<>'' then
    begin
      qReklamlar.Conditions.Add('KOD', 'UPPER(R_DOSYA_ADI) LIKE ''' + Buyuk(SuzgecVerisiIsle(txtDosya.Text)) + '''', True);
      pnlBul.Caption := pnlBul.Caption + ifthen(pnlBul.Caption <> '', ', ') + 'Dosya Adý : ' + SuzgecVerisiIsle(txtDosya.Text);
    end;

    qReklamlar.Conditions.Enable;
    qReklamlar.Open;
...
procedure TReklamlarForm.btnDegistirClick(Sender: TObject);
var
  I: Integer;
  T: TUniTabSheet;
  F: TReklamForm;
begin
  try
    if qReklamlar.IsEmpty then exit;
    if isFrameOpenedInTab('TReklamForm', TUniTabSheet(Self.Parent)) then exit;

    UniMainModule.Tmp_ID := qReklamlar.FieldByName('R_ID').AsInteger;

    T := TUniTabSheet(Self.Parent);
    T.Caption := T.Caption + ' > Dosya Adý [' + qReklamlar.FieldByName('R_DOSYA_ADI').AsString + ']';

    for I := 0 to T.ControlCount-1 do
      T.Controls[I].Visible := False;

    F := TReklamForm.Create(T);
    F.UniPanel1.Caption :=  'Dosya Adý [' + qReklamlar.FieldByName('R_DOSYA_ADI').AsString + ']';
    F.PReklamlarForm := Self;
    F.Align := alClient;
    F.Parent := T;
    F.Show;
  except on E: Exception do
...
procedure TReklamlarForm.btnKapatClick(Sender: TObject);
begin
  UniMainModule.FrameKapat(Self);
end;

procedure TReklamlarForm.btnSecClick(Sender: TObject);
begin
  try
    UniMainModule.FrameKapat(Self);
  except on E: Exception do
...
procedure TReklamlarForm.btnSilClick(Sender: TObject);
begin
  try
    if qReklamlar.IsEmpty then exit;
    sil.Show();
...
procedure TReklamlarForm.btnYeniClick(Sender: TObject);
var
  I: Integer;
  T: TUniTabSheet;
  F: TReklamForm;
begin
  try
    ...
    T := TUniTabSheet(Self.Parent);
    T.Caption := T.Caption + ' > Dosya Adý [Yeni]';

    for I := 0 to T.ControlCount-1 do
      T.Controls[I].Visible := False;

    F := TReklamForm.Create(T);
    F.UniPanel1.Caption := 'Dosya Adý [Yeni]';
    F.PReklamlarForm := Self;
    F.Align := alClient;
    F.Parent := T;
    F.Show;
...
procedure TReklamlarForm.GridAjaxEvent(Sender: TComponent; EventName: string;
  Params: TUniStrings);
begin
  if EventName='kaydet' then
    UniMainModule.ListeKaydet(3, 'Dosyalar', qReklamlar, Grid)
  else
  if EventName='sil' then
    UniMainModule.ListeVarsayilan(20);
end;

procedure TReklamlarForm.GridColumnSort(Column: TUniDBGridColumn;
  Direction: Boolean);
begin
  UniMainModule.Sort(qReklamlar, Column.FieldName, Direction);
end;

procedure TReklamlarForm.GridDblClick(Sender: TObject);
begin
...
procedure TReklamlarForm.GridDrawColumnCell(Sender: TObject; ACol,
  ARow: Integer; Column: TUniDBGridColumn; Attribs: TUniCellAttribs);
begin
  if Column.FieldName='R_DURUM' then
...
procedure TReklamlarForm.silConfirm(Sender: TObject);
begin
  try
    if FileExists('files\reklamlar\' + qReklamlar.FieldByName('R_DOSYA_ADI').AsString) then
    begin
      if DeleteFile('files\reklamlar\' + qReklamlar.FieldByName('R_DOSYA_ADI').AsString) then
      begin
        qReklamlar.Delete;
    ...
    UniMainModule.Focus(txtDosya);
  except on e:exception do
    UniMainModule.Notification('', HataMesaj(e.Message), 2);
  end;
end;

procedure TReklamlarForm.UniFrameAjaxEvent(Sender: TComponent;
  EventName: string; Params: TUniStrings);
begin
  if EventName = 'esc_pressed' then btnKapatClick(Sender);
end;

procedure TReklamlarForm.UniFrameCreate(Sender: TObject);
begin
...
//    Grid.ClientEvents.UniEvents.Values['pagingBar.afterCreate'] := 'function (sender){ sender.dock = "top"; }';
  ...
  try
    ...
    begin
      txtDosya.Text := UniMainModule.Tmp_Kod;
