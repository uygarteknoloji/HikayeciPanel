...
uses ServerModule, MainModule, Utils, Dosya;

procedure TDosyalarForm.btnBulClick(Sender: TObject);
begin
  try
    qDosyalar.Close;
    qDosyalar.Conditions.Clear;
    pnlBul.Caption := '';
    Grid.WebOptions.PageSize := StrToInt(txtKayitSayisi.Text);
    if txtDurum.ItemIndex=0 then
    begin
      qDosyalar.Conditions.Add('DURUM', 'DOS_DURUM=1', True);
    ...
    begin
      qDosyalar.Conditions.Add('KOD', 'UPPER(CIH_ADI) LIKE ''' + Buyuk(SuzgecVerisiIsle(txtCihaz.Text)) + '''', True);
      pnlBul.Caption := pnlBul.Caption + ifthen(pnlBul.Caption <> '', ', ') + 'Cihaz Adý : ' + SuzgecVerisiIsle(txtCihaz.Text);
    end;
    if txtKategori.Text<>'' then
    begin
      qDosyalar.Conditions.Add('KOD', 'UPPER(KAT_ADI) LIKE ''' + Buyuk(SuzgecVerisiIsle(txtKategori.Text)) + '''', True);
      pnlBul.Caption := pnlBul.Caption + ifthen(pnlBul.Caption <> '', ', ') + 'Kategori : ' + SuzgecVerisiIsle(txtKategori.Text);
    end;
    if txtEserAdi.Text<>'' then
    begin
      qDosyalar.Conditions.Add('KOD', 'UPPER(DOS_ESER_ADI) LIKE ''' + Buyuk(SuzgecVerisiIsle(txtEserAdi.Text)) + '''', True);
      pnlBul.Caption := pnlBul.Caption + ifthen(pnlBul.Caption <> '', ', ') + 'Eser Adý : ' + SuzgecVerisiIsle(txtEserAdi.Text);
    end;

    qDosyalar.Conditions.Enable;
    qDosyalar.Open;
...
procedure TDosyalarForm.btnDegistirClick(Sender: TObject);
var
  I: Integer;
  T: TUniTabSheet;
  F: TDosyaForm;
begin
  try
    if qDosyalar.IsEmpty then exit;
    if isFrameOpenedInTab('TDosyaForm', TUniTabSheet(Self.Parent)) then exit;

    UniMainModule.Tmp_ID := qDosyalar.FieldByName('DOS_ID').AsInteger;

    T := TUniTabSheet(Self.Parent);
    T.Caption := T.Caption + ' > Dosya [' + qDosyalar.FieldByName('DOS_ESER_ADI').AsString + ']';

    for I := 0 to T.ControlCount-1 do
      T.Controls[I].Visible := False;

    F := TDosyaForm.Create(T);
    F.UniPanel1.Caption :=  'Dosya [' + qDosyalar.FieldByName('DOS_ESER_ADI').AsString + ']';
    F.PDosyalarForm := Self;
    F.Align := alClient;
    F.Parent := T;
    F.Show;
  except on E: Exception do
...
procedure TDosyalarForm.btnKapatClick(Sender: TObject);
begin
  UniMainModule.FrameKapat(Self);
end;

procedure TDosyalarForm.btnSecClick(Sender: TObject);
begin
  try
    UniMainModule.FrameKapat(Self);
  except on E: Exception do
...
procedure TDosyalarForm.btnSilClick(Sender: TObject);
begin
  try
    if qDosyalar.IsEmpty then exit;
    sil.Show();
...
procedure TDosyalarForm.btnYeniClick(Sender: TObject);
var
  I: Integer;
  T: TUniTabSheet;
  F: TDosyaForm;
begin
  try
    ...
    T := TUniTabSheet(Self.Parent);
    T.Caption := T.Caption + ' > Dosya [Yeni]';

    for I := 0 to T.ControlCount-1 do
      T.Controls[I].Visible := False;

    F := TDosyaForm.Create(T);
    F.UniPanel1.Caption := 'Dosya [Yeni]';
    F.PDosyalarForm := Self;
    F.Align := alClient;
    F.Parent := T;
    F.Show;
...
procedure TDosyalarForm.GridAjaxEvent(Sender: TComponent; EventName: string;
  Params: TUniStrings);
begin
  if EventName='kaydet' then
    UniMainModule.ListeKaydet(3, 'Dosyalar', qDosyalar, Grid)
  else
  if EventName='sil' then
    UniMainModule.ListeVarsayilan(20);
end;

procedure TDosyalarForm.GridColumnSort(Column: TUniDBGridColumn;
  Direction: Boolean);
begin
  UniMainModule.Sort(qDosyalar, Column.FieldName, Direction);
end;

procedure TDosyalarForm.GridDblClick(Sender: TObject);
begin
...
procedure TDosyalarForm.GridDrawColumnCell(Sender: TObject; ACol,
  ARow: Integer; Column: TUniDBGridColumn; Attribs: TUniCellAttribs);
begin
  if Column.FieldName='DOS_DURUM' then
...
procedure TDosyalarForm.UniFrameAjaxEvent(Sender: TComponent;
  EventName: string; Params: TUniStrings);
begin
  if EventName = 'esc_pressed' then btnKapatClick(Sender);
end;

procedure TDosyalarForm.UniFrameCreate(Sender: TObject);
begin
...
//    Grid.ClientEvents.UniEvents.Values['pagingBar.afterCreate'] := 'function (sender){ sender.dock = "top"; }';
  ...
  try
    if qDosyalar.Active then
      qDosyalar.Close;
    qDosyalar.Open;

    UniMainModule.ListeYukle(3, Grid);
    ...
    begin
      txtCihaz.Text := UniMainModule.Tmp_Kod;
