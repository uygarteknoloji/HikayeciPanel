...
type
  TUFirmalarForm = class(TUniFrame)
    UniPanel1: TUniPanel;
...
uses MainModule, ServerModule, Utils, UFirma;

procedure TUFirmalarForm.btnBulClick(Sender: TObject);
begin
  try
    qFirmalar.Close;
...
procedure TUFirmalarForm.btnDegistirClick(Sender: TObject);
var
  I: Integer;
  T: TUniTabSheet;
  F: TUFirmaForm;
...
procedure TUFirmalarForm.btnSecClick(Sender: TObject);
begin
  try
    if PKullaniciForm<>nil then
    begin
...
procedure TUFirmalarForm.btnSilClick(Sender: TObject);
begin
  try
    if qFirmalar.IsEmpty then exit;
    sil.Show();
...
procedure TUFirmalarForm.btnYeniClick(Sender: TObject);
var
  I: Integer;
  T: TUniTabSheet;
  F: TUFirmaForm;
...
procedure TUFirmalarForm.GridAjaxEvent(Sender: TComponent; EventName: string;
  Params: TUniStrings);
begin
...
procedure TUFirmalarForm.GridColumnSort(Column: TUniDBGridColumn;
  Direction: Boolean);
begin
  UniMainModule.Sort(qFirmalar, Column.FieldName, Direction);
end;

procedure TUFirmalarForm.GridDblClick(Sender: TObject);
begin
...
procedure TUFirmalarForm.GridDrawColumnCell(Sender: TObject; ACol,
...
procedure TUFirmalarForm.silConfirm(Sender: TObject);
begin
  try
    qFirmalar.Delete;
...
procedure TUFirmalarForm.UniFrameAjaxEvent(Sender: TComponent;
  EventName: string; Params: TUniStrings);
begin
  if EventName = 'esc_pressed' then btnKapatClick(Sender);
end;

procedure TUFirmalarForm.UniFrameCreate(Sender: TObject);
begin
  ...
  Grid.ClientEvents.UniEvents.Values['pagingBar.beforeInit'] :=
    'function(sender, config){' +
       'sender.displayMsg = "Kayýt Adedi : {2}";' +
       'sender.displayInfo = true;' +
    '}';
//  Grid.ClientEvents.UniEvents.Values['pagingBar.beforeInit'] := 'function (sender, config){' +
//         'sender.displayMsg = "Kayýt Adedi : {2}";' + // defaultvalue = Displaying {0} - {1} of {2}' +
//         'sender.displayInfo = true;' +
//         '}';
...
//    Grid.ClientEvents.UniEvents.Values['pagingBar.afterCreate'] := 'function (sender){ sender.dock = "top"; }';
  ...
  UniMainModule.ButtonMask([btnYeni, btnDegistir, btnGridExport, btnBilgi, btnBul, btnSec, btnKapat]);
